...
export function notificationIcon(type: string): { Icon: typeof Bell; variant: IconBoxVariant } {
  switch (type) {
    ...
    case 'recording_compressed':
      return { Icon: Minimize2, variant: 'success-subtle' }
    case 'site_blocked':
...
export function notificationTarget(n: AppNotification): string | null {
  ...
  if (d.recording_id && (n.type === 'recording_completed' || n.type === 'recording_compressed')) return `/watch/${d.recording_id}`
